...
namespace tinyutils
{
    ...
    {
        if ( !s_Instance )
            s_Instance = std::unique_ptr<Clock>( new Clock() );
        ...
        /// Initialize main clock-event (default event that the clock keeps track of)
        s_Instance->m_ClockEvents[MAIN_EVENT] = ClockEvent();
        s_Instance->m_ClockEvents[MAIN_EVENT].name = MAIN_EVENT;
        s_Instance->m_ClockEvents[MAIN_EVENT].time_start = s_Instance->_TimeStampNow();
        s_Instance->m_ClockEvents[MAIN_EVENT].time_stop = s_Instance->_TimeStampNow();
    ...
    {
        LOG_CORE_ASSERT( s_Instance, "Clock::Tick >>> Must initialize clock-module before using it" );
        s_Instance->_Tick( event_name );
    }

    void Clock::Tock( const std::string& event_name )
    {
        LOG_CORE_ASSERT( s_Instance, "Clock::Tock >>> Must initialize clock-module before using it" );
        s_Instance->_Tock( event_name );
    }

    ClockEvent Clock::GetEvent( const std::string& event_name )
    {
        LOG_CORE_ASSERT( s_Instance, "Clock::GetEvent >>> Must initialize clock-module before using it" );
        if ( s_Instance->m_ClockEvents.find( event_name ) == s_Instance->m_ClockEvents.end() )
        {
            LOG_CORE_WARN( "Clock::GetEvent >>> event named {0} wasn't found on the set of registered clock-events", event_name );
    ...
    {
        LOG_CORE_ASSERT( s_Instance, "Clock::GetTimesBuffer >>> Must initialize clock-module before using it" );
        return s_Instance->m_TimesBuffer;
    }

    Clock::BufferArray Clock::GetFpsBuffer()
    {
        LOG_CORE_ASSERT( s_Instance, "Clock::GetFpsBuffer >>> Must initialize clock-module before using it" );
    ...
    void Clock::_Tock( const std::string& event_name )
    {
        if ( m_ClockEvents.find( event_name ) == m_ClockEvents.end() )
        {
            LOG_CORE_WARN( "Clock::_Tock >>> tried calling _Tock() with a non-started clock-event \"{0}\"", event_name );
            return;
        }
    ...
    double Clock::_TimeStampNow()
    {
        ...
        auto time_stamp = std::chrono::time_point_cast<std::chrono::milliseconds>( time_stamp_chrono ).time_since_epoch().count() * 0.001;
        return time_stamp;
    }
}