...
namespace tinyutils
{
    ...
    void PerlinNoise::Init()
    {
        ...
        for ( ssize_t o = 0; o < s_Instance->m_NumOctaves; o++ )
            s_Instance->m_OctavesOffsets.push_back( std::make_pair( UNIF_RANDOM( -10000.0f, 10000.0f ), 
        ...
        s_Instance->m_Permutations = { 151,160,137,91,90,15,131,13,201,95,96,53,194,233,7,225,140,36,103,30,69,142,
        ...
        s_Instance->m_Permutations.insert( s_Instance->m_Permutations.end(), s_Instance->m_Permutations.begin(), s_Instance->m_Permutations.end() );
    }
    ...
    void PerlinNoise::Config( ssize_t num_octaves, float persistance, float lacunarity, float noise_scale )
    {
        LOG_CORE_ASSERT( s_Instance, "PerlinNoise::Config >>> Must initialize perlin-noise module before using it" );
        s_Instance->_Config( num_octaves, persistance, lacunarity, noise_scale );
    }

    float PerlinNoise::Sample1d( float x )
    {
        LOG_CORE_ASSERT( s_Instance, "PerlinNoise::Sample1d >>> Must initialize perlin-noise module before using it" );
        return s_Instance->_Sample1d( x );
    }

    float PerlinNoise::Sample2d( float x, float y )
    {
        LOG_CORE_ASSERT( s_Instance, "PerlinNoise::Sample2d >>> Must initialize perlin-noise module before using it" );
        return s_Instance->_Sample2d( x, y );
    }

    void PerlinNoise::_Config( ssize_t num_octaves, float persistance, float lacunarity, float noise_scale )
    {
        ...
        for ( ssize_t o = 0; o < m_NumOctaves; o++ )
            m_OctavesOffsets.push_back( std::make_pair( UNIF_RANDOM( -10000.0f, 10000.0f ), 
                                                        UNIF_RANDOM( -10000.0f, 10000.0f ) ) );
    }

    float PerlinNoise::_Sample1d( float x )
    {
        return _Sample2d( x, 0.0f );
    }
    ...
    {
        // Because we are in 2d, there are just 4 options for the gradient vectors and so, just 4 options for the dot product
        switch ( hash & 0xf )
        {
            case 0x0 : return x + y;
            case 0x1 : return -x + y;
