...
#define DEFAULT_SESSION "session_default"
...
namespace tinyutils
{
    ...
    private :
        ...
        std::chrono::time_point<std::chrono::high_resolution_clock> m_TimePointStart;
    };

    /// Interface for profiling sessions, which are used to handle profiling results (saving to disk, etc.)
    ...
    public :
        ...
        {
            /// Internal type of session, stores all sessions results in a buffer for internal tools to use
            INTERNAL,
            /// External-chrome type of session, saves to disk (.json) all session results in a format for the chrome-tracing tool to read
            EXTERNAL_CHROME
        };
        ...
        enum class eState : uint8_t
        {
            ...
            IDLE,
            /// Running state, session is waiting and saving|storing the provided results
            RUNNING
        };
        ...
        /// Sends this session to its running state (to keep track of profiling results)
        virtual void Begin() = 0;

        /// Writes profiling result (to disk, stores it, etc.) depending of profiler type
        virtual void Write( const ProfilerResult& result ) = 0;

        /// Sends this session to its idle state (stops keeping track of profiling results)
        virtual void End() = 0;
    ...
    /// Profiling session that stores results for later usage of internal tooling
    ...
    /// Profiling session that saves the results to disk in the chrome-tracing tool required format
    ...
    public :
        ...
        /// Appends the result into a .json file for later inspection using chrome's tracing tool
        void Write( const ProfilerResult& result ) override;
    ...
    private :
        ...
        /// Dictionary container for all sessions created during the module's lifetime
        std::unordered_map<std::string, std::unique_ptr<IProfilerSession>> m_Sessions;

        /// Type of the profiler-sessions created (either INTERNAL, or EXTERNAL_CHROME)
        IProfilerSession::eType m_ProfilerType;
    };
}

#define PROFILE_SCOPE(name)                             tinyutils::ProfilerTimer prof_timer##__LINE__(name, DEFAULT_SESSION)
#define PROFILE_SCOPE_IN_SESSION(name, session_name)    tinyutils::ProfilerTimer prof_timer##__LINE__(name, session_name)

#define PROFILE_FUNCTION()                          tinyutils::ProfilerTimer prof_timer##__LINE__(__FUNCTION_NAME__, DEFAULT_SESSION)
#define PROFILE_FUNCTION_IN_SESSION(session_name)   tinyutils::ProfilerTimer prof_timer##__LINE__(__FUNCTION_NAME__, session_name)